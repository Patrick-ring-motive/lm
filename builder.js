...
(async () => {
  ...
  function buildNGrams(text, n = 3, type = "normal") {
    ...
    for (let i = 0; i < tokens.length - n + 1; i++) {
      ...
      if (i % 50000 === 0 && i > 0) log('buildNGrams: progress', {
        i,
  ...
  function buildPrunedNGrams(text, n = 3) {
    ...
    for (let i = 0; i < tokens.length - n + 1; i++) {
      ...
      model[key] ??= {};
      model[key][next] = (model[key][next] || 0) + 1;
      if (i % 50000 === 0 && i > 0) log('buildPrunedNGrams: progress', {
        i
      });
    }
    for (const key in model) {
  ...
  function parseDoc(input) {
    log('parseDoc: parsing input length', {
      len: input?.length
    });
  ...
  if (typeof process) {
    ...
    async function writeFile(filePath, content) {
      ...
      (async () => {
        ...
        let texts = (
          await Promise.all([
            ...
            readFile("hobbit.txt"),
            readFile("hobbit_cleaned.txt"),
            readFile("hobbit_enhanced.txt"),
            readFile("fellowship.txt"),
            readFile("fellowship_cleaned.txt"),
            readFile("fellowship_enhanced.txt"),
            //readFile("../tolkienizer/fellowship.txt"),
            readFile("towers.txt"),
            readFile("towers_cleaned.txt"),
            readFile("towers_enhanced.txt"),
            readFile("king.txt"),
            readFile("king_cleaned.txt"),
            readFile("king_enhanced.txt"),
            //readFile("chars_cleaned.txt"),
            readFile("third_age_cleaned.txt"),
      ...
      getDocText("https://pjhobbitfilms.fandom.com/wiki/The_Hobbit:_An_Unexpected_Journey/Transcript"),*/
        ...
        log('main: file reads complete', {
          files: texts.length
        });

        let texts2 = texts.map(text => text.replaceAll('-', ' ').replaceAll('—', ' ').replaceAll('�', "'"));
        let texts3 = texts.map(text => text.replace(/\s+[-]\s+/g, '_').replace(/\s+[—]\s+/g, '_'));
        let allTexts = texts.concat(texts2).concat(texts3).map(x => x
          .replaceAll('`', ' ')
          .replaceAll(':', ",")
          .replace(/\s+s\s+/g, "'s ")
          .replace(/\s+es\s+/g, "'es ")
        ...
        log('main: trimodel built', {
          keys: Object.keys(trimodel).length
        });
        // trimodel = Object.fromEntries(Object.entries(trimodel).sort());
        let bimodel = (mergeModels(...allBimodels));
        log('main: bimodel built', {
          keys: Object.keys(bimodel).length
        });
