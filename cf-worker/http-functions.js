...
export function get_probe1() {
  try {
    ...
    const info = handles.map((h, i) => ({
      ...
      keys: (() => {
        try {
          return Reflect.ownKeys(h).slice(0, 25).map(String);
        } catch {
          return [];
        }
      })(),
...
export function get_probe2() {
  try {
    ...
    const info = reqs.map((r, i) => ({
      ...
      keys: (() => {
        try {
          return Reflect.ownKeys(r).slice(0, 20).map(String);
        } catch {
          return [];
        }
      })(),
          ...
          protoChain: (() => {
            ...
            while (p && p !== Object.prototype) {
              chain.push({
                ...
                keys: Reflect.ownKeys(p).slice(0, 20).map(String)
              });
...
export async function get_probe9() {
  ...
  if (!raw) {
    return serverError({
      headers: CORS,
      body: {
        error: 'Could not find raw ServerResponse'
      }
    });
  }
